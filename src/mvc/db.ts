...
export async function getOneOrNull<T>(whereQuery: DB.Query) {
  const result = checkHasResult<DB.IQueryResult>(
    await whereQuery.limit(1).get()
  )
  const { data } = result
...
export async function tryUpdate(
  whereQuery: DB.Query, updateContent = {}, 
  autoUpatedTime = true
) {
  if (autoUpatedTime) {
...
export async function updateByDoc(
  doc: DB.DocumentReference, updateContent = {}, 
  autoUpatedTime = true
) {
  if (autoUpatedTime) {
    updateContent = updateTime(updateContent)
  ...
  const result = checkHasResult<DB.IUpdateResult>(
    await doc.update({ data: updateContent })
  )
  return result.stats.updated
}
